import { StrayCat, WalletListItem } from '@/types/WalletType'
...
function getWalletTypeOrder(item: WalletListItem) {
...
function getTypeOrder(item: WalletListItem) {
  switch (item.type) {
    case 'WALLET':
      return 0
    case 'CAT_LIST':
...
export default function useWalletsList(
  ...
  function isHiddenCAT(assetId: string) {
    ...
    // NOTE : the wallet id may be undefined, so now use 0 as temp but it is not sure
    return isHidden(walletId ?? 0)
  }
  ...
  async function handleShow(id: number | string) {
    try {
      ...
      if (typeof id === 'string') {
        // NOTE: if the stray cat is still in the list,then return
        if (wallets?.find((wallet) => wallet.meta.assetId === id)) {
          return id
        }
